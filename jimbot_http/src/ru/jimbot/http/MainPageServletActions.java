...
//    }
    ...
    protected String prefToHtml(Object o) {
        ...
        for (int i = 0; i < p.length; i++) {
            String c = p[i].getPropertyType().getSimpleName();
            if(p[i].isHidden()) {
                // пропускаем
                ...
                try {
                    DbConfig d = (DbConfig) PropertyUtils.getProperty(o, p[i].getName());
                    s += "<TR><TH ALIGN=LEFT>" + PropertyUtils.getPropertyDescriptor(d, "host").getDisplayName() + "</TD> " +
                            "<TD><INPUT size=\"70\" TYPE=text NAME=\"" + p[i].getName() + ".host" +
                            "\" VALUE=\"" + PropertyUtils.getProperty(d, "host") + "\"></TD></TR>";
                    s += "<TR><TH ALIGN=LEFT>" + PropertyUtils.getPropertyDescriptor(d, "base").getDisplayName() + "</TD> " +
                            "<TD><INPUT size=\"70\" TYPE=text NAME=\"" + p[i].getName() + ".base" +
                            "\" VALUE=\"" + PropertyUtils.getProperty(d, "base") + "\"></TD></TR>";
                    s += "<TR><TH ALIGN=LEFT>" + PropertyUtils.getPropertyDescriptor(d, "user").getDisplayName() + "</TD> " +
                            "<TD><INPUT size=\"70\" TYPE=text NAME=\"" + p[i].getName() + ".user" +
                            "\" VALUE=\"" + PropertyUtils.getProperty(d, "user") + "\"></TD></TR>";
                    s += "<TR><TH ALIGN=LEFT>" + PropertyUtils.getPropertyDescriptor(d, "pass").getDisplayName() + "</TD> " +
                            "<TD><INPUT size=\"70\" TYPE=text NAME=\"" + p[i].getName() + ".pass" +
                            "\" VALUE=\"\"></TD></TR>";
                } catch (Exception e) {
                    e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
                }
